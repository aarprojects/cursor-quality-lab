...
def test_retrieve_payment_intent(stripe_context: APIRequestContext, test_customer):
    ...
    create = stripe_context.post("/v1/payment_intents", form={
        "amount": "5000",
        "currency": "usd",
        "customer": test_customer["id"],
        "payment_method_types[]": "card",
    ...
    get = stripe_context.get(f"/v1/payment_intents/{pi_id}")
...
def test_cancel_payment_intent(stripe_context: APIRequestContext, test_customer):
    ...
    pi_id = create.json()["id"]
    cancel = stripe_context.post(f"/v1/payment_intents/{pi_id}/cancel")
