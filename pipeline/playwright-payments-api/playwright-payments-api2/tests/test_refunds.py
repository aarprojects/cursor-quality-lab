...
def confirmed_charge(stripe_context: APIRequestContext):
    response = stripe_context.post("/charges", form={
        "amount": "5000",
        "currency": "usd",
        "source": "tok_visa",
...
def test_full_refund(stripe_context: APIRequestContext, confirmed_charge):
    ...
    response = stripe_context.post("/refunds", form={
...
def test_duplicate_refund_fails(stripe_context: APIRequestContext, confirmed_charge):
    ...
    stripe_context.post("/refunds", form={"charge": confirmed_charge["id"]})
    second = stripe_context.post("/refunds", form={"charge": confirmed_charge["id"]})
